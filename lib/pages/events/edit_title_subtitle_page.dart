...
import '../../widgets/cell_group_picker.dart';
            ...
            child: ListTile(
              ...
              subtitle: Text(
                parentHead?.subtitle.trim().isNotEmpty == true
                    ? parentHead!.subtitle
                    : (parentID == null
                        ? 'Tap to attach under a period parent'
                        : 'id: $parentID'),
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
              ),
      ...
      childrenOf: (id) {
        if (id == widget.eventContext.id) {
          return List<String>.from(
              widget.eventContext.metadata.childrenPostIDs);
        }
        final cached = appContext.getMetadata(id);
        return cached == null
            ? const <String>[]
            : List<String>.from(cached.childrenPostIDs);
      },
    );
    if (createsCycle) {
      if (!mounted) return;

...
- **Cell Groups** — planning; handoff in [`docs/cell-groups.md`](docs/cell-groups.md). Durable CG entities + main nav (tiered privacy); `CellGroupIDs` on posts/templates; season parents; companion user model: placeholder `users` + `CreatedByUserID` scoped Auth link. Continue with: “Continue cell groups from `docs/cell-groups.md`”.
